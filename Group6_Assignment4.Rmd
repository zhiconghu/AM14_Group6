...
2.  Principle Component factor are not supported by economic intuition while Fama-French 3 factors is much more explainable due to its nature of being interpretable.
...
**Load data containing daily yield curve data for the United States between July 2 1981 and January 31 2020**
...
pca_variance %>% ggplot(aes(fct_reorder(PC, -`Proportion of Variance`), `Proportion of Variance`)) +
  theme_bw() +
...
From the plot we can see that 2-3 components are enough to explain the majority of variations, with PC1 explaining 98.38%, PC2 01.53% and PC3 0.65%.
...
```{r 8.4, warning=FALSE, message=FALSE}
ggplot(rotation, aes(x=time)) + 
  ...
  geom_point(aes(y = PC3), shape=21, color="darkgreen", fill="darkgreen", size=1.5) +
  
  xlab("Spot rate time periods (years)")+
  ylab("Principal component ")+
  labs(title = "The first three eigenvectors for changes in the treasury yields.", subtitle = "Time series plot of first three principal components. PC1 in red, PC2 in blue, PC3 in red.")+
  theme_bw() 
...
PCA is a useful statistical method to summarize information but PCA cannot answer the important question of what economic forces cause prices to move. As such, correlations
