...
joined_by_region %>% ggplot(aes(x = fct_rev(fct_reorder(region, operating_profits)), y = operating_profits)) +
  geom_col() +
...
```{r}
joined_by_region %>% ggplot(aes(x = fct_rev(fct_reorder(region, gross_profits)), y = gross_profits)) +
  geom_col() +
  labs(x = "Region", y = "Gross Profits")
  theme_minimal()
...
As shown in figure 1, the contribution margin is higher for Jefferey Alexander brand.
...
p <- head(transactions_by_collection) %>% ggplot(aes(x = contribution_margin, y = fct_reorder(collection, contribution_margin))) + 
  geom_col() +
  geom_text(aes(label = round(contribution_margin, 2)))
