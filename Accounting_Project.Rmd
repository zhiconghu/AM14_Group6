...
```{r}
transactions_by_brand %>% ggplot(aes(x = contribution_margin, y = brand, fill = brand)) + 
  geom_col() +
  geom_text(aes(label = round(contribution_margin, 2)),  hjust = 1.5, color = '#ffffff') +
  scale_fill_manual(values = c("darkred", "darkblue"))+
  labs(x = "Contribution Margin", y = "Brand") +
  theme_minimal()
```
...
transactions_by_brand %>% ggplot(aes(x = sales_revenue, y = brand, fill = brand)) + 
  geom_col() +
