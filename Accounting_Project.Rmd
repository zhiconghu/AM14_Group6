...
overheads_by_region <-overheads %>%  pivot_longer(2:9,names_to = "region", values_to = "overheads") %>% 
  ...
  group_by(region) %>% 
  summarise(production_overheads = sum(production_overheads), 
            non_production_overheads = sum(non_production_overheads))  #summing up overhead costs across all regions
overheads_by_region
...
```{r}
joined_by_region <- joined_by_region %>% mutate(overhead_weight = production_overheads/sales_revenue)

joined_by_region %>% summarise(sum(production_overheads))

transactions_by_brand <- transactions %>% 
  left_join(joined_by_region[c(1,8)], by = "region") %>% 
  mutate(production_overheads = overhead_weight*sales_revenue) %>% 
  group_by(brand) %>%
  summarize(costs = sum(costs), sales_revenue = sum(sales_revenue), production_overheads = sum(production_overheads)) %>% 
  mutate(contribution_margin = 1- (costs+production_overheads)/sales_revenue)

transactions_by_brand %>% 
  summarise(sum(production_overheads))
